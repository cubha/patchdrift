...
| ST-05 | 통계 엔진 — Wilson CI, Newcombe 두 비율 차이 CI, BH-FDR, beta-binomial 축소, 최소 n 게이트 판정 | `src/pipeline/aggregate/stats.ts` + 테스트(기지값 대조) | F2 | B2 [P] |
...
| ST-08 | 델타·1단 결정론 짝짓기·판정 + Data Dragon 매핑 — 두 패치 집계 diff → `DeltaRecord`(Δ·CI·q·n) → 엔티티 ID 블로킹(ko 이름↔ddragon key) → 방향 정합 → `MatchStatus` 4종 → `data/aggregated/deltas/{from}_{to}.json` · `scripts/run-ddragon.ts`(챔피언/아이템 JSON·이미지 → `public/dd/`) | `src/pipeline/match/{entity-match,verdict,ddragon}.ts` `scripts/run-ddragon.ts` + 테스트 | F4 | B4 [S] |
...
| ST-10 | 웹 데이터 로더·공통 UI — `src/lib/data.ts`(빌드 타임 JSON 로드·패치 쌍 목록), `format.ts`, `Header`·`FilterBar`(정적 선택→라우트)·`StatusBadge`·`DeltaValue`·`EntityIcon`(public/dd) — 프로토타입 공통 마크업 1:1 | `src/lib/*` `src/components/*` `src/app/layout.tsx` + 테스트(format) | F5 | B5 [S] |
...
| ▶ 운영 | B2 검증 후 **실수집 시작**(백그라운드): 26.16·26.17 상세 각 ≥2,000(목표 1만) + 타임라인 표본 — D+1 게이트(gameVersion 포맷·startTime·challenges 필드 확인) | — | 수집 3.3h/패치, 구현과 병행 |
...
- `data/raw/{patch}/matches.jsonl` — `MatchSlim` 1행/매치 · `data/raw/{patch}/timelines.jsonl` — `TimelineSlim` · `data/raw/{patch}/seen-ids.txt`
